...
package com.msht.mshtlpgmaster.handler;
...
import com.msht.mshtlpgmaster.thread.MyDecodeThread;
...
public final class MyScanDecodeHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        ...
        if (message.what == R.id.decode) {
            Log.d(TAG, "handleMessage: 在DecodeThread的DecodeHandler中执行decode");
            decode((byte[]) message.obj, message.arg1, message.arg2);
        } else if (message.what == R.id.quit) {
            Log.d(TAG, "handleMessage: quit 终止DecodeHandler");
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        if (rawResult != null) {
            ...
            Log.d(TAG, "decode: decode_failed 子线程的DecodeHandler发送decode失败消息给主线程CptureFragment的CaptureActivityHandler");
            message.sendToTarget();
        }
    }
